...
`setup/ace-custom-spring-wrapper.md` is **archived** — see `docs/archive/deprecated-ace-deployment/`. The Spring Boot wrapper deployment path is no longer active.
...
- [`llm-context-authoring.md`](llm-context-authoring.md) — guidance for what belongs in `SKILL.md` versus retrieval-backed `internalKBs`
...
- [`configuration.md`](configuration.md) — HAT accessibility config only (remainder of this file has been stripped; see security-audit.md for config layering)
...
- [`superpowers/plans/`](superpowers/plans/) — implementation plans generated by the superpowers planning tool (tool-managed location)
- [`superpowers/specs/`](superpowers/specs/) — architecture design specs from superpowers (tool-managed location)
