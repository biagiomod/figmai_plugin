# Archive Policy

**Purpose:** This directory (`docs/archive/`) contains historical documentation that is no longer authoritative for the current state of the FigmAI plugin.

---

## 1. What "Archive" Means

- **Historical Reference Only:** Documents here describe past decisions, completed phases, or superseded plans. They are kept for context and historical traceability.
- **Not Authoritative:** For current architectural guidance, implementation details, or feature specifications, always refer to the main documentation index ([docs/README.md](../README.md)).
- **Outdated Terminology:** Terminology within archived documents may reflect older conventions (e.g., "Work Plugin" instead of "Custom Plugin"). Always use the current terminology defined in the main documentation when discussing current features.


---

## 2. Structure

- **Root-level (this directory):** Archived plans, audits, and reports moved from `docs/` with original filenames preserved (including ALL_CAPS). Examples: ADMIN_CONFIG_EDITOR_PLAN_V2.md, ADMIN_CONFIG_EDITOR_IMPLEMENTATION_PLAN.md, DESIGN_CRITIQUE_ASSISTANT_AUDIT_AND_MIGRATION_PLAN.md, DESIGN_WORKSHOP_ASSISTANT_AUDIT_AND_MIGRATION_PLAN.md, ERRORS_* audits, LLM_PROMPT_* audits, ARCHITECTURE_AUDIT_ASSISTANTS.md, ANALYTICS_TAGGING_ACTIONID_AUDIT.md, PROJECT_CONTEXT_FROM_CHATGPT.md, PR11c1-hardening-and-PR11c2-plan.md, PR11c1-QA-evidence.md. Each has an "Status: Archived" banner at the top.
- **phases/:** Summaries and reports for completed development phases.
- **verification/:** Detailed verification reports for completed features.
- **qa/:** Historical QA plans and test commands.
- **refactor-tickets/:** Documentation related to past refactoring efforts.
- **strategic/:** Older strategic planning documents.
- Other root-level `.md` files (e.g., CODEBASE_AUDIT.md, REFACTOR_AUDIT.md) are also historical.

---

## 3. Finding Current Documentation

Always start at the main documentation index: [FigmAI Plugin Documentation Index](../README.md)

For current ACE migration and hosting topics, use:

1. [architecture/ace-static-s3-migration.md](../architecture/ace-static-s3-migration.md)
2. current code in `infra/config-api/`, `admin-editor/`, and `scripts/`
3. [setup/ace-public-replica.md](../setup/ace-public-replica.md)
