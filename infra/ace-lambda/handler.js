...
const { getModel, validate, saveModel, publishModel } = require('./configService');
...
const AUTH_PATHS = new Set([
  '/api/auth/me',
  '/api/auth/bootstrap-allowed',
  '/api/auth/bootstrap',
  '/api/auth/bootstrap/create-first-account',
  '/api/auth/login',
  '/api/auth/logout',
]);
...
exports.handler = async (event) => {
  ...
  const requestId =
    event.requestContext?.requestId || `req-${Date.now()}`;
  ...
  if (!PUBLIC_PATHS.has(path) && !AUTH_PATHS.has(path)) {
    if (!isAuthorized(event)) {
      logRequest({ requestId, method, path, status: 401, latencyMs: Date.now() - startMs });
      return addRequestId(json(401, { error: 'Unauthorized' }, origin), requestId);
    }
  }
  ...
  try {
    ...
    response = json(500, { error: err.message || 'Internal server error' }, origin);
  }

  response.headers = { ...response.headers, 'x-request-id': requestId };
  logRequest({ requestId, method, path, status: response.statusCode, latencyMs: Date.now() - startMs });
  return response;
};
...
async function dispatch(method, path, body, requestId, origin) {
  ...
  // Auth
  if (path.startsWith('/api/auth/')) return handleAuth(method, path, body, origin);

  // Config
  if (method === 'GET' && path === '/api/model') return getModel(origin);
  if (method === 'POST' && path === '/api/validate') return validate(body, origin);
  if (method === 'POST' && path === '/api/save') return saveModel(body, requestId, origin);
  if (method === 'POST' && path === '/api/publish') return publishModel(requestId, origin);

  // Knowledge bases
  if (path.startsWith('/api/kb')) return handleKb(method, path, body, requestId, origin);

  // Users
  if (path.startsWith('/api/users')) return handleUsers(method, path, body, requestId, origin);

  // Test routes
  if (path.startsWith('/api/test/')) return handleTest(method, path, body, origin);
...
function handleHealth(origin) {
  ...
  return json(200, {
    ...
    storage: { type: 's3', bucket: info.bucket, prefix: info.prefix, region: info.region },
  }, origin);
}
...
function logRequest({ requestId, method, path, status, latencyMs }) {
  console.log(JSON.stringify({ type: 'request', requestId, method, path, status, latencyMs }));
}
