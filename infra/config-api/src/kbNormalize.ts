...
export function parseMarkdown(md: string, id: string, titleOverride?: string): KnowledgeBaseDocument {
  ...
  for (const { level, name, body } of sections) {
    ...
    if (key === 'definitions') doc.definitions = extractListItems(body)
    if (key === 'rulesConstraints') doc.rulesConstraints = extractListItems(body)
    if (key === 'do') doc.doDont.do = extractListItems(body)
    if (key === 'dont') doc.doDont.dont = extractListItems(body)
    if (key === 'examples') {
      const fenced = extractFencedBlocks(body)
      doc.examples = fenced.length > 0 ? fenced : extractListItems(body)
    }
    if (key === 'edgeCases') doc.edgeCases = extractListItems(body)
  }
