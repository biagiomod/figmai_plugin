...
export async function publishResponse(requestId) {
    ...
    if (rawMeta) {
        try {
            draftVersion = Number(JSON.parse(rawMeta).version || 0);
        }
        catch {
            draftVersion = 0;
        }
    ...
    await putObjectText(`snapshots/${snapshotId}/_manifest.json`, `${JSON.stringify(manifest, null, 2)}\n`, 'application/json');
    ...
    await putObjectText(snapshotPublishedKey, `${JSON.stringify(published, null, 2)}\n`, 'application/json');
    await putObjectText('published.json', `${JSON.stringify(published, null, 2)}\n`, 'application/json');
    logAction({
        requestId,
        action: 'publish',
        detail: {
