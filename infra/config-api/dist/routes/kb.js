...
import { KB_ID_REGEX, knowledgeBaseDocumentSchema } from '../schemas';
...
async function writeRegistry(registry) {
    await putObjectText('draft/knowledge-bases/registry.json', `${JSON.stringify(registry, null, 2)}\n`, 'application/json');
}
function getKbPath(id) {
    return `draft/knowledge-bases/${id}.kb.json`;
}
async function upsertRegistryEntry(doc) {
    ...
    if (idx >= 0)
        registry.knowledgeBases[idx] = entry;
    else
...
export async function getKbByIdResponse(id) {
    ...
    try {
        ...
        return { statusCode: 500, payload: { error: e.message } };
    }
}
...
export async function normalizeKbResponse(body) {
    ...
    try {
        ...
        const json = JSON.parse(parsed.data.content);
        return {
            statusCode: 200,
        ...
        return {
            statusCode: 200,
            payload: { errors: [e instanceof SyntaxError ? 'Invalid JSON' : e.message] }
        };
    }
}
...
export async function createKbResponse(body, requestId) {
...
export async function patchKbResponse(id, body, requestId) {
    const parsedId = idParamSchema.safeParse({ id });
    if (!parsedId.success) {
        return {
            statusCode: 400,
...
export async function deleteKbResponse(id, requestId) {
    const parsedId = idParamSchema.safeParse({ id });
    if (!parsedId.success) {
        return {
            statusCode: 400,
