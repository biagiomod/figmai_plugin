/**
 * Analytics Tagging Assistant Handler (ARCHIVED v1)
...
import type { AssistantHandler, HandlerContext, HandlerResult } from '../../assistants/handlers/base'
import { getAbsoluteBounds } from '../../stage/anchor'
import { validateTargetOnly, validateContainerForDraft } from './selectionV1'
import { captureVisibleInArea } from './screenshotV1'
import { loadSession, saveSession, createNewSession } from '../storage'
import type { Row, Session, DraftRow } from '../types'
...
export class AnalyticsTaggingHandlerV1 implements AssistantHandler {
  ...
  async handleResponse(context: HandlerContext): Promise<HandlerResult> {
    ...
    if (actionId === 'capture-screenshot-add-row') {
      ...
      let ref: NonNullable<Row['screenshotRef']>
      try {
