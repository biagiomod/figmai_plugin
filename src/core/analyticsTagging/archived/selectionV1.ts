/**
 * Analytics Tagging — single-node validation and root screen derivation (ARCHIVED v1)
...
export async function validateTargetOnly(selectedNode: SceneNode): Promise<ValidateTargetOnlyResult> {
  ...
  const screenIdResult = await readScreenIdFromNode(rootScreen, rootScreen.name)
  const actionIdResult = await readActionIdFromNode(target)
