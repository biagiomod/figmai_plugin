...
@ActiveProfiles("dev")
class AuthControllerDevStubTest {

    @Autowired
    private MockMvc mockMvc;

    @Test
    void meReturnsAceCompatibleShapeInDevStubMode() throws Exception {
    ...
    @Test
    void bootstrapAllowedReturnsWrapperMessage() throws Exception {
        mockMvc.perform(get("/api/auth/bootstrap-allowed"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.allowed").value(false))
                .andExpect(jsonPath("$.reason").value("Authentication is handled by the Spring wrapper."));
    }
}
